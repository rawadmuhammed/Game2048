...
/**
 * DarkView displays the Game in a dark theme that is easy on the eyes
...
public class DarkView extends PApplet implements ViewInterface{
        public static void main(String[] args) {PApplet.main(DarkView.class);}
    /**
    ...
    final int Y_SIZE  = 2*Y_POS+2*Y_OFFSET+SIZE_BORDER+4*(SIZE_TILE+SIZE_BORDER);
        ...
        public DarkView(){setSize(X_SIZE, Y_SIZE);}
        public void setup() {
            cI = new Controller(this);
            startLogo = loadImage("startLogoDark.png");
    ...
    public void draw() {cI.refreshGame();}
        public void show(int[] grid) {
            ...
            for (int y=0; y<edge_length; y++) {
                ...
                for (int x=0; x<edge_length; x++) {
                    ...
                    fill(color(20+log(grid[i]+1)/log(2)*10, 100, 80));
        ...
        public void startScreen(){
            ...
            textSize(21);
            fill(30, 90, 75);
            text("Drücke eine Pfeiltaste zum Starten", width/2, height-30);
        }
